...
from ghostAgents import DirectionalGhost, RandomGhost  # noqa
...
class MultiagentTreeState(object):
    ...
    def getLegalActions(self, agentIndex=0):
        if VERBOSE:
            print(
                "getLegalActions(%s) -> %s" % (self.state, self.problem.stateToActions[self.state])
            )
    ...
    def getAction(self, state):
        ...
        for agent in self.solutionAgents:
            optimalActionLists.append(
                (agent.getBestPacmanActions(state)[0], len(GameState.getAndResetExplored()))
            )
        alternativeDepthLists = [
            agent.getBestPacmanActions(state)[0] for agent in self.alternativeDepthAgents
        ]
        partialPlyBugLists = [
...
class EvalAgentTest(testClasses.TestCase):
    def __init__(self, question, testDict):
        ...
        self.nonTimeoutMinimum = (
            int(testDict["nonTimeoutMinimum"]) if "nonTimeoutMinimum" in testDict else None
        )
        ...
        self.maxPoints = sum(
            [len(t) for t in [self.scoreThresholds, self.nonTimeoutThresholds, self.winsThresholds]]
        )
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        results = [
            gradeThreshold(averageScore, self.scoreMinimum, self.scoreThresholds, "average score"),
            gradeThreshold(
                nonTimeouts,
                self.nonTimeoutMinimum,
                self.nonTimeoutThresholds,
