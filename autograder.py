...
def readCommand(argv):
    ...
    parser.add_option(
        "--edx-output", dest="edxOutput", action="store_true", help="Generate edX output files"
    )
    parser.add_option(
        "--mute", dest="muteOutput", action="store_true", help="Mute output from executing tests"
    )
    parser.add_option(
        "--print-tests",
        "-p",
...
if __name__ == "__main__":
    ...
    while codePaths:
        ...
        for cp in codePaths[:]:
            ...
            try:
                moduleObj = loadModuleFile(moduleName, os.path.join(options.codeRoot, cp))
            except ImportError as e:
                error = e
            else:
                moduleDict[moduleName] = moduleObj
                loaded_module = True
                codePaths.remove(cp)
        if not loaded_module:
            raise Exception(
                "Cound not load the following student modules: {}.\nThe previous error was '{}: {}'".format(
                    codePaths, type(error).__name__, error
                )
            )
